    ...
    private static int locate(CommandContext<CommandSourceStack> ctx) {
        ...
        CompletableFuture.supplyAsync(() -> search(seed, originX, originZ, radius, generator, level, randomState, seaLevel))
                .whenComplete((result, error) -> level.getServer().execute(() -> {
                    ...
                    int dist = (int) Math.sqrt((double) (result.x - originX) * (result.x - originX)
                            + (double) (result.z - originZ) * (result.z - originZ));
                    source.sendSuccess(() -> Component.literal("Chasm found at ")
                            .withStyle(ChatFormatting.GREEN)
                            .append(Component.literal(result.x + ", ~, " + result.z)
                                    .withStyle(ChatFormatting.GOLD))
                            .append(Component.literal(" (" + dist + " blocks away, about "
                                    + Math.round(result.halfWidth * 2) + " blocks wide there)")
                                    .withStyle(ChatFormatting.GRAY)), false);
                }));
        return 1;
    }
